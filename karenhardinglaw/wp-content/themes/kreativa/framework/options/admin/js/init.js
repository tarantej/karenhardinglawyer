jQuery(document).ready( function($) {
  ...
  function GoogleFontSelect( slctr, mainID ){
   ...
   // Insta Reset!
   $('#reset-instagram-cache').live("click", function(){
      //console.log(formdata);
      $.ajax({
        type: "post",
        url: ajaxurl,
        data: 'action=mtheme_insta_reset',
    beforeSend : function(){
    },
    success: function(data){
      $('#insta-reset-Message').fadeIn();
    },
    error: function(jqXHR, textStatus, errorThrown) {
      $('#insta-error-Message').fadeIn();
    }
      }); 
      
      setTimeout("jQuery('#insta-reset-Message,#insta-error-Message').fadeOut();", 4000);
