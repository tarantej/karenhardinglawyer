...
final class Noakes_Menu_Manager_Field_Select extends Noakes_Menu_Manager_Field
{
	...
	private function _radio_buttons()
	{
		...
		{
			$output .= '<label><input' . str_replace($id, $id . '-' . ++$index, $this->input_attributes) . ' type="radio" value="' . esc_attr($value) . '" ' . checked($this->value, $value, false) . $classes . ' />' . $label . '</label>';
		}
	...
	private function _select_box()
	{
		...
		foreach ($this->options as $value => $label)
		{
			if (is_array($label))
			{
				...
				{
					$output .= '<option value="' . esc_attr($group_value) . '" ' . selected($this->value, $group_value, false) . '>' . $group_label . '</option>';
				}
