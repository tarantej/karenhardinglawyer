...
class mtheme_SidebarGallery_Widget extends WP_Widget {
	function __construct() {
		...
		parent::__construct('mtheme_sidebar_gallery', __('Kreativa Gallery Widget'), $widget_ops);
	}
		...
		if ( !empty( $title ) ) { 
			...
			switch ($listchoice) {
				...
				case "portfolio" :
					...
					$thumbnail_image_id = get_post_thumbnail_id(($post->ID), 'kreativa-gridblock-tiny'); 
				...
				case "page" :
					if ( function_exists('kreativa_get_custom_attachments') ) {
						$filter_image_ids = kreativa_get_custom_attachments ( $thepage );
					}
					...
					if ( isSet($filter_image_ids) && isSet($thepage) ) {
						...
						$thumbnail_imagearray = wp_get_attachment_image_src( $attatchmentID , 'kreativa-gridblock-tiny', false);
				...
				case "category" :
					...
					$thumbnail_image_id = get_post_thumbnail_id(($post->ID), 'kreativa-gridblock-tiny'); 
					$thumbnail_image_url = wp_get_attachment_image_src($thumbnail_image_id,'kreativa-gridblock-tiny'); 
					$thumbnail_image_url = $thumbnail_image_url[0];
